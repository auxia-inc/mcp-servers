...
const TOKEN_FILE = path.join(CONFIG_DIR, 'gdrive-token.json');
const CALLBACK_PORT = 3035;
          ...
          res.end(`
            <html>
              <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                ...
                <p>Google Drive MCP has been authorized.</p>
...
export async function getAuthenticatedClient(autoPopup: boolean = true): Promise<AuthResult> {
  ...
  if (!token && autoPopup) {
    ...
    throw new Error(
      'Google Drive not authenticated. Use the authenticate tool or run: npm run auth'
    );
  }
 ...
 */
async function main() {
  console.log('Google Drive MCP Authentication');
  console.log('================================\n');
  ...
  try {
    ...
    console.log(`\nVerified: Authenticated as ${about.data.user?.emailAddress}`);
  } catch (error) {
