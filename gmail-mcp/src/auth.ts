...
const TOKEN_FILE = path.join(CONFIG_DIR, 'gmail-token.json');
const CALLBACK_PORT = 3037;
          ...
          res.end(`
            <html>
              <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                ...
                <p>Gmail MCP has been authorized.</p>
...
export async function getAuthenticatedClient(autoPopup: boolean = true): Promise<AuthResult> {
  ...
  if (!token && autoPopup) {
    ...
    throw new Error(
      'Gmail not authenticated. Use the authenticate tool or run: npm run auth'
    );
  }
 ...
 */
async function main() {
  console.log('Gmail MCP Authentication');
  console.log('========================\n');
  ...
  try {
    ...
    console.log(`\nVerified: Authenticated as ${profile.data.emailAddress}`);
  } catch (error) {
